...
def get_google_folders_from(url):
    ...
    try:
        ...
        print("Unable to get_google_folders_from: ", repr(e))
    return rows
        ...
        for bld in builds:
            ...
            if b > int(build):
                if "TEST" in bld.get("comment"):
                    continue
                mainWindow.after(100, mainWindow.SendConsoleMessage("A newer version of the firmware is available. Please update."))
...
def get_game_profiles(url, mainWindow):
    ...
    result.sort() #key=lambda x: x[0])
    # display result
    mainWindow.SendConsoleMessage("Processed " + str(len(result))+ " game and voice files\r\n") # game_name
    return result
